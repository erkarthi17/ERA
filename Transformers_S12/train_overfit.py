...
def evaluate(model, data_loader, device, use_amp=False, max_batches=None):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            # but we also have max_batches to prevent infinite loop if logic is slightly off
...
def train(args):
    ...
    param_count = sum(p.numel() for p in model_local.parameters())
    ...
    print(f"Initial loss: {val_loss:.6f}")
...
def make_arg_parser():
    ...
    p.add_argument('--ckpt_path', type=str, default='ckpt_overfit.pth')
